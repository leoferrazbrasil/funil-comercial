...
- **Privadas:** `/dashboard` `/inbox` `/contatos` `/leads` `/funil` `/campanhas` `/criativos` `/roteiro` `/perfil` `/configuracoes`. Acesso por papel em `src/lib/accessControl.ts` (vendedor não vê criativos/roteiro).
