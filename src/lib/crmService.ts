...
import type { Contact, CrmSnapshot, InboxMessage, Lead, Opportunity, OpportunityStage, Profile } from './types'
...
export async function ensureDefaultStages(ownerId: string) {
  const supabase = requireSupabase()
  const { data, error } = await supabase.from('pipeline_stages').select('id').eq('owner_id', ownerId).limit(1)
  if (error) throw error
...
export async function getCrmSnapshot(userId: string): Promise<CrmSnapshot> {
  ...
  const [profileResult, contactsResult, leadsResult, opportunitiesResult, messagesResult] = await Promise.all([
...
export async function createOpportunity(ownerId: string, payload: OpportunityPayload) {
  const supabase = requireSupabase()
  const { data, error } = await supabase
    .from('opportunities')
    .insert({
...
export async function createInboxMessage(ownerId: string, payload: MessagePayload) {
  const supabase = requireSupabase()
  const { data, error } = await supabase
    .from('inbox_messages')
    .insert({
