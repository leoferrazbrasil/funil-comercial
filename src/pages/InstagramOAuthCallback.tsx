...
// Callback do login pelo Instagram (/oauth/instagram). Troca o code por token via
// a Edge Function instagram-auth e avisa a janela-mãe (PublishModal) no sucesso.
export default function InstagramOAuthCallback() {
  const [status, setStatus] = useState<"loading" | "success" | "error">("loading");
  const [message, setMessage] = useState("Conectando com o Instagram...");
    ...
    const exchange = async () => {
      try {
        const { data, error: fnError } = await supabase.functions.invoke("instagram-auth", {
          body: { code, redirectUri: `${window.location.origin}/oauth/instagram` },
        });
