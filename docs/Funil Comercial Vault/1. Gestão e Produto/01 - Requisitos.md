...
- **Área segura por formato (canvas, `SAFE_AREA`/`<SafeFrame>`):** textos, logos e CTAs ficam na faixa central; só decorativos sangram até a borda. As margens de conteúdo **excedem o mínimo crop-safe da Meta** (34px laterais / 60px topo-base no 4:5), dando respiro. **4:5:** 64px laterais / 72px topo-base. **9:16 (1080×1920):** **200px** de topo livres (nome do perfil) + base reservada (~250px, ajustável) à legenda/áudio/ações · laterais 64px. **1:1:** 64px simétrico. A **headline auto-ajusta** o tamanho (`fitHeadline`, máx 110px no `t1` / 96px no `t4`) para caber na largura segura sem cruzar as margens, mesmo com tema longo.
