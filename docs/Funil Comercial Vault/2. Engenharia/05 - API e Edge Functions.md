...
|---|---|---|
| `whatsapp-inbound` | Webhook de mensagens recebidas (Z-API/Meta). Meta: verificação GET (`hub.challenge`), assinatura (`META_APP_SECRET`), parsing `entry→changes→value→messages`; resolve o dono pelo `phone_number_id`. Também processa `statuses[]` (entrega/leitura/falha → selinho no Inbox). Captura o **`ctwa_clid`** do `referral` (anúncio Click-to-WhatsApp) e persiste em `leads`/`contacts` — ver [[CTWA - Atribuicao e Conversions API]]. Resolve o canal (`channel_id`) tolerando variação do 9º dígito — ver [[Inbox - Incidente Canal Ausente (channel_id)]]. Ignora grupos/newsletters para manter a Inbox como operação um-a-um — ver [[Inbox - Grupos e Conversas Novas Hoje]]. | **false** |
